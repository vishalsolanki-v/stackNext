...
const Theme = () => {
    ...
    return (<Menubar className="relative border-none bg-transparent shadow-none">
        ...
        dark:focus:bg-dark-200">
                {mode==='light' ? (
                    <Image src='/assets/icons/sun.svg' alt='sun'
                        width={20}
                        height={20}
                        className='active-theme'/>) : 
                    (
                        <Image src='/assets/icons/moon.svg' alt='Moon'
                            width={20}
                            height={20}
                            className='active-theme'/>)}
            </MenubarTrigger>
        ...
        dark:border-dark-400 dark:bg-dark-300">
                {
                    themes.map((theme)=>(
                        <MenubarItem key={theme.value} onClick={()=>{setMode(theme.value)
                            ...
                            <Image src={theme.icon} alt={theme.value} height={16} width={16}
                                className={`${mode===theme.value&&'active-theme'}`}/>
                            <p className={`body-semibold text-light-500 ${mode===theme.value? 
                                'text-primary-500':'text-dark100_light900'}`}>{theme.label}</p>
                        </MenubarItem>
                    ))
                }
            </MenubarContent>
